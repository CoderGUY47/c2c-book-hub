...
const AuthPage: React.FC<LoginProps> = ({isLoginOpen, setIsLoginOpen}) => {
    ...
    const onSubmitSignUp = async(data: SignUpFormData)=>{
        ...
        try {
            ...
            if(result.success){
                toast.success("Registration verification link sent! Please verify your email before logging in.");
    ...
    <Dialog open={isLoginOpen} onOpenChange={setIsLoginOpen}>
      <DialogContent className='sm:max-w-[500px] p-6 bg-white/10 backdrop-blur-lg border border-white/20 shadow-lg rounded-lg'>
        <DialogHeader>
            <DialogTitle className='text-center font-bold text-white text-2xl mt-4 mb-4'>
                    ...
                    >
                        ...
                        <TabsContent value='login' className='space-y-4'>
                            <form className="space-y-4" onSubmit={handleLoginSubmit(onSubmitLogin)}>
                                ...
                                {loginError.email && (
                                    <p className='text-indigo-300 text-sm'>{loginError.email.message}</p>
                                )}
                                <div className="relative text-white mb-4">
                                    <Input
                                ...
                                {loginError.password && (
                                    <p className='text-indigo-300 text-sm'>{loginError.password.message}</p>
                                )}
                        ...
                        <TabsContent value='signup' className='space-y-4'>
                            <form onSubmit={handleSignUpSubmit(onSubmitSignUp)} className="space-y-4 ">
                                ...
                                {signupError.password && (
                                    <p className='text-indigo-300 text-sm'>{signupError.password.message}</p>
                                )}
                                ...
                                {signupError.agreeTerms && (
                                    <p className='text-indigo-300 text-sm'>{signupError.agreeTerms.message}</p>
                                )}
                        ...
                        <TabsContent value='forgot-password' className='space-y-4'>
                            ...
                            <form className="space-y-4" onSubmit={handleForgotPasswordSubmit(onSubmitForgotPassword)}>
                                ...
                                {forgotPasswordError.email && (
                                    <p className='text-indigo-300 text-sm'>{forgotPasswordError.email.message}</p>
                                )}
            ...
            <p className='text-center text-sm text-gray-300 mt-2'>By clicking "Send Password Reset Link", you agree to our {""}
